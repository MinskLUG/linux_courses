...
\begin{frame}{Практика: определение типа файла}
  ...
  \begin{itemize}
    \item Используем \alert{ls -l}
      \lstinputlisting[frame=single]{samples/ls-l}
      Первая колонка, первый символ \pause

    \item Определение типа файла: \alert{file}
      \lstinputlisting[frame=single]{samples/file-usage}
  \end{itemize} 
\end{frame}
...
\lstinputlisting[frame=single,basicstyle=\small]{samples/cat-input-file} \pause
