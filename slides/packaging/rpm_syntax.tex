	...
	\frametitle{Подготовка окружения для сборки}	
			...
			\end{itemize}			
				\begin{table}
					...
					\hline
					~/rpmbuild/SPECS/ & \%\_specdir & Сборочные .spec файлы\\
					~/rpmbuild/SOURCES/ & \%\_sourcedir & Исходные архивы и патчи\\
					\hline
					~/rpmbuild/BUILD/ & \%\_builddir	& Сборка\\		
					~/rpmbuild/BUILDROOT/ & \%\_buildrootdir & Установка\\
					\hline
					~/rpmbuild/RPMS/ & \%\_rpmdir & Собранные .rpm\\
					~/rpmbuild/SRPMS/ & \%\_srcrpmdir & ``Собранный'' .srpm \\
					\end{tabular}
				\end{table}			
			\begin{block}{Выполнить команды}
				{\tt rpmdev-setuptree ; cd ~/rpmbuild/SPECS ; vim program.spec }
			\end{block}			
\end{frame}
