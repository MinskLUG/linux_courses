...
<xsl:output omit-xml-declaration="yes" encoding="UTF-8" use-character-maps="returns" /> 
		...
		<xsl:text>}&#xD;</xsl:text>
			...
			<xsl:choose>
				<xsl:when test="current()/node/@TEXT">
					<xsl:choose>
						<xsl:when test="current()/node/richcontent/html/body/img or current()/node/richcontent/html/body/p/img ">
...
<xsl:template name="itemization">
	...
	<xsl:for-each select="$i">
		<xsl:choose>
			...
			<xsl:when test="@TEXT and not(../cloud) and not(../../cloud)">
				<xsl:text>	\item </xsl:text><xsl:value-of select="@TEXT" disable-output-escaping="yes"/>
				<xsl:text>&#xD;</xsl:text>
				<xsl:if  test="current()/hook">
					<xsl:text>&#xA;\begin{equation*}&#xA;&#x9;</xsl:text>
